Meteor.methods({
  loginWithWx: function(args) {
    ...
    if (json.openid) {
      var user = Meteor.users.findOne({ openid: json.openid });
      ...
      if (user) {
        ...
        var url = "https://api.weixin.qq.com/sns/userinfo?access_token=" + json.access_token + "&openid=" + json.openid + "&lang=zh_CN ";
        // console.log(url)
        var res = HTTP.get(url);
        ...
        var user = {
          username: CryptoJS.MD5(url).toString(),
          emails:[{address:CryptoJS.MD5(url).toString()+'@demo.com'}],
          password: CryptoJS.MD5(url).toString(),
          "openid": info.openid,
          "nickname": info.nickname,
          "avatar": info.headimgurl,
          "unionid": info.unionid,
          "createAt": new Date(),
          "applyid": CryptoJS.MD5(new Date().getTime() + "").toString().substring(0, 2).toUpperCase() + parseInt(Math.random() * 10000 % 90000 + 10000),
        };
        // console.log(user)
...
getWxToken = function() {
  ...
  if (ack) {
    ...
    if (response.statusCode === 200) {
      var wx = wxaccesstoken.findOne();
      wxaccesstoken.update({ _id: wx._id }, {
